...
export function transitionCheckIn(
  from: CheckInStatus,
  to: CheckInStatus,
): CheckInStatus {
  if (!transitions[from].includes(to)) {
    throw new DomainError(
      "INVALID_CHECK_IN_TRANSITION",
...
export function assertInsideCheckInWindow(
  now: Date,
  opensAt: Date,
  closesAt: Date,
): void {
  if (now < opensAt || now > closesAt) {
    throw new DomainError(
      ...
      "Este check-in está fuera de plazo.",
    );
  }
}
...
export function assertReviewer(options: {
  ...
  if (options.reviewerId === options.ownerId) {
    throw new DomainError(
      ...
      "No puedes validar tu propia evidencia.",
    );
  }
